...
float equatorialPlane = 3.14159265358979323846;	// Equatorial plane angle in radians
...
vec3 getPolar(vec3 cartesian)
{
	...
	float theta = acos(cartesian.y / r);		// GLSL uses .x, .y, .z for vector components
	float phi = atan(cartesian.z, cartesian.x);	// atan(y, x) is used instead of atan2
...
void main()
{
	vec3 vel = updateVelocity(getPolar(vec3(0, 0, 1)), getPolar(vec3(0, 0, -OriginalDistance)));
	...
	vec3 camPos = getPolar(vec3(0, 0, -OriginalDistance));//vec3((OriginalDistance - (Speed * iTime)) * sin(iTime * camSpeed), 0, (OriginalDistance - (Speed * iTime)) * cos(iTime * camSpeed)); // Position of the camera
	...
	// vec3(-1, 0, 0);//
	vec3 rayDir = vec3(-sqrt(1-normUV.x*normUV.x-normUV.y*normUV.y), normUV);//normalize(camDir + (camRight * ndc.x * tan(fovXRad / 2.0)) + (camUpReal * ndc.y * tan(fovYRad / 2.0)));

	// Use this ray direction with the camera's position to cast rays into the scene
	vec3 rayOrigin = getPolar(vec3(normUV.x, normUV.y, -OriginalDistance));//camPos;
